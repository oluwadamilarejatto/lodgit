    ...
    <div style={{ minHeight: "100vh", background: "#080F1A", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Sora','Segoe UI',sans-serif", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ background: "#0D1B2A", borderRadius: 20, padding: 28, border: "1px solid #1E293B" }}>
          <h2 style={{ color: "#F1F5F9", fontSize: 20, fontWeight: 800, margin: "0 0 6px" }}>
          ...
          <button onClick={handleEmail} disabled={loading} style={{ width: "100%", padding: 13, background: "linear-gradient(135deg,#FF6B35,#FF4D8F)", border: "none", borderRadius: 10, color: "#fff", fontWeight: 800, fontSize: 15, cursor: "pointer", fontFamily: "inherit", opacity: loading ? 0.7 : 1 }}>
...
// ── Workspace Screen ─────────────────────────────────────────────────────
function WorkspaceScreen({ user, onReady }) {
  ...
  const create = async () => {
    ...
    const { data, error } = await supabase.from("workspaces").insert([{
      name: wsName.trim(), type: wsType, code,
    ...
    await supabase.from("workspace_members").insert([{
      ...
      email: user.email, role: "admin", joinedAt: new Date().toISOString()
    }]);
  ...
  const join = async () => {
    ...
    setLoading(true); setError("");
    const { data, error } = await supabase.from("workspaces").select("*").eq("code", joinCode.toUpperCase());
    if (error || !data?.length) { setError("Invalid code. Check with your team lead."); setLoading(false); return; }
    const ws = data[0];
    await supabase.from("workspace_members").insert([{
      workspaceId: ws.id, userId: user.id,
      name: user.user_metadata?.full_name || user.email,
      email: user.email, role: "member", joinedAt: new Date().toISOString()
    }]);
    localStorage.setItem("lodgit_workspace", JSON.stringify(ws));
    onReady(ws);
    setLoading(false);
    ...
    <div style={{ minHeight: "100vh", background: "#080F1A", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Sora','Segoe UI',sans-serif", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 400 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <p style={{ color: "#475569", fontSize: 13, margin: 0 }}>Welcome, {user.user_metadata?.full_name?.split(" ")[0] || "there"}! 👋</p>
        </div>
        ...
        <div style={{ background: "#0D1B2A", borderRadius: 20, padding: 28, border: "1px solid #1E293B" }}>
          {mode === "choose" && (
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {[
                  { m: "create", icon: "🏢", title: "Create a Workspace", sub: "For your team or personal use", color: "#FF6B35" },
                  { m: "join",   icon: "🔗", title: "Join a Workspace",   sub: "Your team lead shared an invite code", color: "#3B9EFF" },
                ].map(item => (
                  <button key={item.m} onClick={() => setMode(item.m)} style={{ padding: 18, background: "#0F172A", border: "1px solid #1E293B", borderRadius: 14, textAlign: "left", cursor: "pointer" }}
                    onMouseEnter={e => e.currentTarget.style.borderColor = item.color}
                    onMouseLeave={e => e.currentTarget.style.borderColor = "#1E293B"}
                  >
                    <div style={{ fontSize: 24, marginBottom: 6 }}>{item.icon}</div>
                    <div style={{ color: "#F1F5F9", fontWeight: 700, fontSize: 14, marginBottom: 3 }}>{item.title}</div>
          ...
          {mode === "create" && (
            <>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <input value={wsName} onChange={e => setWsName(e.target.value)} placeholder="e.g. First Bank Ikeja, My Clinic" style={inp} />
              </div>
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[["team", "👥 Team"], ["personal", "👤 Personal"]].map(([val, label]) => (
                    <button key={val} onClick={() => setWsType(val)} style={{ flex: 1, padding: "10px 8px", background: wsType === val ? "#FF6B3522" : "#0F172A", border: `1.5px solid ${wsType === val ? "#FF6B35" : "#1E293B"}`, borderRadius: 10, cursor: "pointer", color: wsType === val ? "#FF6B35" : "#F1F5F9", fontWeight: 700, fontSize: 13, fontFamily: "inherit" }}>
                      {label}
                    </button>
                  ))}
                </div>
              ...
              <button onClick={create} disabled={loading || !wsName.trim()} style={{ width: "100%", padding: 13, background: wsName.trim() ? "linear-gradient(135deg,#FF6B35,#FF4D8F)" : "#1E293B", border: "none", borderRadius: 10, color: wsName.trim() ? "#fff" : "#334155", fontWeight: 800, fontSize: 14, cursor: wsName.trim() ? "pointer" : "not-allowed", fontFamily: "inherit" }}>
          ...
          {mode === "join" && (
            <>
              ...
              <div style={{ marginBottom: 20 }}>
                <input value={joinCode} onChange={e => setJoinCode(e.target.value.toUpperCase())} placeholder="e.g. AB12CD" style={{ ...inp, textAlign: "center", fontSize: 20, fontWeight: 800, letterSpacing: 4 }} maxLength={6} />
              </div>
              {error && <p style={{ color: "#FF4444", fontSize: 12, margin: "0 0 14px" }}>{error}</p>}
              <button onClick={join} disabled={loading || joinCode.length < 6} style={{ width: "100%", padding: 13, background: joinCode.length >= 6 ? "linear-gradient(135deg,#3B9EFF,#A855F7)" : "#1E293B", border: "none", borderRadius: 10, color: joinCode.length >= 6 ? "#fff" : "#334155", fontWeight: 800, fontSize: 14, cursor: joinCode.length >= 6 ? "pointer" : "not-allowed", fontFamily: "inherit" }}>
...
// ── Workspace Code Banner ────────────────────────────────────────────────
export function WorkspaceCodeBanner({ workspace, onDismiss }) {
  const [copied, setCopied] = useState(false);
  const copy = () => { navigator.clipboard.writeText(workspace.code); setCopied(true); setTimeout(() => setCopied(false), 2000); };
  return (
    <div style={{ background: "#FF6B3511", border: "1px solid #FF6B3533", borderRadius: 12, padding: "14px 16px", margin: "0 14px 12px", display: "flex", alignItems: "center", gap: 12 }}>
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: 11, color: "#475569", marginTop: 2 }}>Share with your team to join {workspace.name}</div>
      </div>
      <button onClick={copy} style={{ background: "#FF6B35", border: "none", borderRadius: 8, color: "#fff", padding: "8px 12px", fontWeight: 700, fontSize: 12, cursor: "pointer", flexShrink: 0 }}>{copied ? "Copied!" : "Copy"}</button>
...
export default function LodgitAuth({ children }) {
  ...
  if (!workspace) return <WorkspaceScreen user={session.user} onReady={handleWorkspaceReady} />;
